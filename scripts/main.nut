...
function onScriptLoad()
{
    ...
    if(arr.len()>=4)
    {
        ...
        if(arr2.len()>=2) if(arr2[1]=="true") ConvertXmlFix=-1;
    }
    ...
    if(arr.len()>=5)
    {
        ...
        if(arr2.len()>=2) if(AutoSaveTime>=300) AutoSaveTime=arr2[1].tointeger();
    }
    if(arr.len()>=6)
    {
...
function onPlayerCommand(player,cmd,text)
{
    ...
    else if(cmd=="p")
    {
        ...
        MessagePlayer(""+player.Angle+"",player);
    }

    else if(cmd=="fly")
    {
        if(state[player.ID].fly==false) state[player.ID].fly=true;
        else state[player.ID].fly=false;
        MessagePlayer("[#00FF00]Your fly mode is ("+state[player.ID].fly+")",player);
    }

    else if(cmd=="autosave")
    {
        if(AutoSaveOn==false) AutoSaveOn=true;
    ...
    else if(cmd=="delallveh")
    {
        ...
        else MessagePlayer("[#FF0000]There are no vehicles on the server.",player);
    }
    ...
    else if(cmd=="selectobj"||cmd=="selobj"||cmd=="objsel")
    {
        if(GetObjectCount()>=1)
        {
            ...
            if(text&&text=="m") 
            {
                ...
                {
                    local find=false;
                    for(local i=0;i<state[player.ID].objarr.len();i++) if(object.ID==state[player.ID].objarr[i]) find=true;
                    if(find==false) 
                    {
...
function onObjectShot(object,player,weapon)
{
    ...
    {
        if(state[player.ID].obj!=null) if(object.ID==state[player.ID].obj.ID) return false;
        state[player.ID].obj=object;
        UpdateObjInfo(player.ID);
        MessagePlayer("[#00FF00]The object with ID "+object.ID+" is selected!",player);
...
function onKeyDown(player,key)
{
    ...
    if(key==Key_RButton) if(state[player.ID].keyw==true) CreateVehForPlr(player.ID,"191");
    if(key==Key_LButton) 
    {
        ...
        if(state[plr.ID].fly==true)
        {
            if(state[plr.ID].obj==null)
            {
                ...
                else
                {
                    ...
                    if(state[plr.ID].frozen==true) if(plr.Frozen==false) plr.Frozen=true;
                }
            }
            else
            {
...
function onKeyBreak(i)
{
	...
	if(plr)
	{
        ...
        if(state[plr.ID].frozen==true) if(plr.Frozen==true) plr.Frozen=false;
	}
}
...
function CheckObjNotEditedForPlr(i,p)
{
    ...
    {
        local find=false;
        if(state[plr.ID].obj!=null) if(state[plr.ID].obj.ID==obj.ID) find=true;
        if(state[plr.ID].objarr!=null)
        {
            for(local i=0;i<state[plr.ID].objarr.len();i++)
            {
    ...
    if(plr)
    {
        if(state[plr.ID].objarrsee==true)
        {
            ...
            if(state[plr.ID].objarr!=null)
            {
                ...
                {
                    local obj=FindObject(state[plr.ID].objarr[i]);
                    if(obj) if(i<=state[plr.ID].objarrlen) str+=""+obj.Pos.x+","+obj.Pos.y+","+obj.Pos.z+",";
                }
            }
